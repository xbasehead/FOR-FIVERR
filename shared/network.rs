...
impl CombinedBehaviour {
    ...
    pub fn add_explicit_peer_to_mesh(&mut self, topic: &IdentTopic, peer_id: PeerId) {
        self.gossipsub.add_explicit_peer(&peer_id);
        println!("Added peer {} to gossipsub mesh for topic {}", peer_id, topic);
    }

    pub fn get_subscribed_peers(&self, topic: &IdentTopic) -> Vec<PeerId> {
        self.gossipsub.topics().filter_map(|(t, peers)| {
            if t == &topic.hash() {
                Some(peers.clone())
            } else {
                None
...
impl Network {
    ...
    pub fn add_explicit_peer_to_mesh(&mut self, topic: &IdentTopic, peer_id: PeerId) {
        self.swarm.behaviour_mut().add_explicit_peer_to_mesh(topic, peer_id);
    }

    pub fn get_subscribed_peers(&self, topic: &IdentTopic) -> Vec<PeerId> {
        self.swarm.behaviour().get_subscribed_peers(topic)
    }
}
...
pub async fn setup_network(config: NetworkConfig) -> Result<Network, Box<dyn std::error::Error>> {
    ...
    let gossipsub_config = gossipsub::ConfigBuilder::default()
        .heartbeat_interval(Duration::from_secs(1)) // Reduced from 10s to 1s
        .mesh_n(4)
        .mesh_n_low(3)
        .mesh_n_high(5)
        .fanout(4) // Increased from default 3
        .build()
        .map_err(|e| {
